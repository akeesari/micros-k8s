...
In this lab, I will guide you through the process of creating Docker container for SQL Server database and run SQL Server database in the docker, and finally accessing the SQL Server database using `SSMS` and `Azure Data Studio` tools.
...
- **Step-5:** Test the SQL Server database connection from psql tool
- **Step-6:** Test the SQL Server database from pgadmin4 tool
...
- SQL Server Management Studio installed - this will allow you to manage the database
- Azure Data Studio installed - this will allow you to connect to SQL server database
...
Inside our Git repository, create a new directory or folder specifically for your SQL Server database. This folder will contain all the necessary files for SQL Server database, including database scripts, docker compose & Dockerfile and other psql files.
...
```sh
docker run -d -p 1433:1433 --name your-container-name your-image-name
```

This will create and run a Docker container with SQL Server running inside it. Adjust the port mapping (`-p 1433:1433`) and container name (`--name your-container-name`) as needed.
...
## Step-5: Test the SQL server database connection from SSMS
