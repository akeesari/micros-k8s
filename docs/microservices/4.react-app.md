...
As a `Frontend (FE)` developer you've been asked to create a website / UI application using React JS technology which is one of the small Website (UI) in the MicroFrontend applications list.
 
This lab will help you to understand how to start your journey by crating React JS application as part of the Microservices Architecture, we will start with existing git repo here and create new React JS project and finally containerize this Website and push to the Azure container registry (ACR) for AKS deployment.

We are basically preparing a React JS application for the Kubernetes deployment. The UI applications we are going to create here in this lab will be used in the subsequent labs like while creating DevOps pipelines or while deploying to Azure Kubernetes services (AKS).
