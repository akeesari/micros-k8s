...
Ensuring the security of our website is importenant and a crucial. and make sure that we implement HTTPS through a valid SSL/TLS certificate. There are various Certificate Authorities (CAs) available, and in this guide, we'll leverage the power of Let's Encrypt, a renowned CA offering free certificates.

This walkthrough will demonstrate how to issue a Cert-Manager Certificate using Let's Encrypt for your Argocd website's domain URL. By utilizing Terraform, a powerful infrastructure-as-code tool, we'll streamline the process, making the implementation of HTTPS a seamless and automated forever.

## Technical Scenario

As a `Cloud Engineer`  you have been asked to provision certificates for `ArgoCD` domain URL deployed within a Kubernetes environment. Adding to the challenge, there's a strict requirement for these certificates to be obtained from reputable certificate authorities, such as Let's Encrypt, and undergo fully automated renewal every three months.
...
5. **Step 5:** Add new DNS zone record for Argocd custom domain

6. **Step 6:** Verify Certificates

7. **Step 6:** Access ArgoCD via HTTPS
...
Cert-Manager is a Kubernetes add-on that helps with the management of certificates. We've already taken care of this in our previour lab. [Setup Cert-Manager in AKS using Terraform](./5-cert-manager.md)
...
Installing ArgoCD in AKS is done in different lap - [Install ArgoCD CLI](./../devops/argocd/install-argocd-cli.md)
